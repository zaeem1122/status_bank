...
class _StatusTabPage2State extends State<StatusTabPage2>
  ...
  Future<void> _loadFilesFromSavedUri() async {
    ...
    try {
      ...
      final List<dynamic> files = await platform.invokeMethod(
          'getFilesFromUri',
          {'uri': savedFolderUri, 'isBusiness': 'true'}
      );
  ...
  Future<Uint8List?> _loadImageBytes(String uriString) async {
    ...
    try {
      ...
      if (bytes != null) {
        _mediaCache[uriString] = bytes as Uint8List;
  ...
  Future<String?> _getVideoFilePath(String uriString) async {
    ...
    try {
      ...
      if (await localFile.exists()) {
        _mediaCache[uriString] = localPath;
        return localPath;
      }
  ...
  // 🆕 Save to Gallery (Android 10+ compatible) - Business WhatsApp
        ...
        return Card(
          ...
          child: _MediaThumbnailBusiness(
            ...
            onDownload: () {
              ...
              saveToGallery(fileMap); // 🆕 Changed to saveToGallery
            },
            onShare: () async {
              if (!_isPremium) {
                InterstitialService.show();
...
// ✅ Separate stateful widget for Business WhatsApp thumbnails
class _MediaThumbnailBusiness extends StatefulWidget {
  final String uri;
  final bool isVideo;
  ...
  @override
  State<_MediaThumbnailBusiness> createState() => _MediaThumbnailBusinessState();
}

class _MediaThumbnailBusinessState extends State<_MediaThumbnailBusiness> with AutomaticKeepAliveClientMixin {
  dynamic _cachedData;
